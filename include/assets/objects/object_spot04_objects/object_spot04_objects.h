extern u64 gDekuTreeMouthTex_0000[];
extern u64 gDekuTreeMouthTex_0400[];
extern Vtx object_spot04_objectsVtx_000800[];
extern Gfx gDekuTreeMouthDL[];
extern CamData gDekuTreeMouthColCamDataList[];
extern SurfaceType gDekuTreeMouthColSurfaceType[];
extern CollisionPoly gDekuTreeMouthColPolygons[];
extern Vec3s gDekuTreeMouthColVertices[];
extern CollisionHeader gDekuTreeMouthCol;
extern u64 gDekuTreeUnusedMouthTex_0EC0[];
extern u64 gDekuTreeUnusedMouthTex_12C0[];

